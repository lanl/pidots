...
INTEGER, DIMENSION(:), ALLOCATABLE :: cpak
! File Names
...
!   vitmx  => V-Cycle maximum number of iterations
!   bitmx1 => Parallel block Jacobi pre-correction maximum number of iterations
!   bitmx2 => Parallel block Jacobi post-correction maximum number of iterations
!   tolr   => Tolerence for Convergence Check: determine whether abs or rel difference
READ(7,*) err, vitmx, bitmx1, bitmx2, tolr
...
! Cells per dimension per sub-domain (check makes sure they're evenly divisible)
nx = nxt/npx
ny = nyt/npy
nz = nzt/npz
...
CALL MPI_BCAST(ipak,15,MPI_INTEGER,irank,MPI_COMM_WORLD,ierr)
CALL MPI_BCAST(cpak,i,MPI_INTEGER,irank,MPI_COMM_WORLD,ierr)
