...
! Force number of cells per sub-domain to be constant for all sub-domains
ELSE IF (MOD(nxt,npx) /= 0 .OR. MOD(nyt,npy) /= 0 .OR. MOD(nzt,npz) /= 0) THEN
   WRITE(8,'(/,3X,A)') "ERROR: Processors must divide evenly into each respective dimension."
   iexit = iexit + 1
