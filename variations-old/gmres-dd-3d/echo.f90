...
WRITE (8,'(//,1X, A)') title
WRITE (8,106) "AHOT-N-ITMM-3D Method with Parallel GMRES"
IF (tpose == 1) WRITE(8,106) "ITMM operators constructed in transpose form."
IF (om == 0) THEN
   WRITE(8,106) "Modified Gram-Schmidt Arnoldi Method"
ELSE
   WRITE(8,106) "Classical Gram-Schmidt Arnoldi Method"
END IF
IF (pcf == 1) THEN
   IF (pinv == 0) THEN
      WRITE(8,106) "Apply diagonal block preconditioner with factor and system solve."
   ELSE
      WRITE(8,106) "Apply diagonal block preconditioner with inversion and multiplication."
   END IF
END IF
...
WRITE (8,105) "Maximum number of parallel GMRES iterations (angular flux) = ", itmx
WRITE (8,105) "Number of iterations before restart (Restart Number) = ", rn
...
DO g = 1, ng
   DO k = 1, nzt
      DO j = 1, nyt
         ...
         WRITE (8,108) (st(i,j,k,g), i = 1, nxt)
      END DO
   END DO
END DO
