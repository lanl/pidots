...
!
! Use Classical Gram Schmidt orthogonalization
...
REAL*8, DIMENSION(rn) :: cs, sn, y, tmph
...
if (pcf == 1) then
   ! Call for preconditioner now
   call precond(pinv,tpose,rs)
   ! the result of this essentially gives a new value of phi which could be
   ! applied to psi
   ! assume transpose
   do j = 1, 8
      do i = 1, bcs
         ra(i,j) = dot_product(jpsi(:,i,j),rs)
      end do
   end do
   ra = ra + ba
else
...
!      IF (pcf == 1) CALL precond(pinv,tpose,ws)
      ...
      DO i = 1, j
         ...
         END DO
      END DO
      CALL MPI_ALLREDUCE(h(1:j,j),tmph(1:j),j,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
      h(1:j,j) = tmph(1:j)
      DO i = 1, j
         ws = ws - h(i,j)*vs(:,i)
         wa = wa - h(i,j)*va(:,:,i)
...
!      ! Reorthogonalize:  Gram-Schmidt prone to severe cancellation.
!      DO i = 1, j
!         ht(i,j) = DOT_PRODUCT(ws,vs(:,i))
!         DO k = 1,8
!            ht(i,j) = ht(i,j) + DOT_PRODUCT(wa(:,k),va(:,k,i))
!         END DO
!      END DO
!      CALL MPI_ALLREDUCE(ht(1:j,j),tmph(1:j),j,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
!      ht(1:j,j) = tmph(1:j)
!      DO i = 1, j
!         ws = ws - ht(i,j)*vs(:,i)
!         wa = wa - ht(i,j)*va(:,:,i)
!      END DO
!      h(1:j,j) = h(1:j,j) + ht(1:j,j)
