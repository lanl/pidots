SUBROUTINE afcm(i,j,k,xs,ys,zs,xe,ye,ze,incx,incy,incz,n,wt,oct,kp,ks)
...
CALL conk(n,i,j,k,incx,incy,incz,xs,ys,zs,wt,ktmpz,ktmpy,ktmpx)
indx = (n-1)*xys
kp(indx+1:indx+xys,jeq,oct) = kp(indx+1:indx+xys,jeq,oct) + ktmpz
indx = apo*xys + (n-1)*xzs
kp(indx+1:indx+xzs,jeq,oct) = kp(indx+1:indx+xzs,jeq,oct) + ktmpy
indx = apo*(xys+xzs) + (n-1)*yzs
kp(indx+1:indx+yzs,jeq,oct) = kp(indx+1:indx+yzs,jeq,oct) + ktmpx
...
IF (k == ze) THEN
   ...
   DO jj = ys, j, incy
      DO ii = xs, i, incx
         ieq = ii + (jj-1)*sdnx
         ks(ieq,jeq,n,oct) = zbcxy(ii,jj,i,j)
      END DO
   END DO
   DO kk = zs, k, incz
      DO ii = xs, i, incx
         ieq = xys + ii + (kk-1)*sdnx
         ks(ieq,jeq,n,oct) = ybcxy(ii,kk,i,j)
      END DO
   END DO
   DO kk = zs, k, incz
      DO jj = ys, j, incy
         ieq = (xys+xzs) + jj + (kk-1)*sdny
         ks(ieq,jeq,n,oct) = xbcxy(jj,kk,i,j)
      END DO
   END DO
END IF
...
IF (j == ye) THEN
   ...
   DO jj = ys, j, incy
      DO ii = xs, i, incx
         ieq = ii + (jj-1)*sdnx
         ks(ieq,jeq,n,oct) = zbcxz(ii,jj,i)
      END DO
   END DO
   DO kk = zs, k, incz
      DO ii = xs, i, incx
         ieq = xys + ii + (kk-1)*sdnx
         ks(ieq,jeq,n,oct) = ybcxz(ii,kk,i)
      END DO
   END DO
   DO kk = zs, k, incz
      DO jj = ys, j, incy
         ieq = (xys+xzs) + jj + (kk-1)*sdny
         ks(ieq,jeq,n,oct) = xbcxz(jj,kk,i)
      END DO
   END DO
END IF
...
IF (i == xe) THEN
   ...
   DO jj = ys, j, incy
      DO ii = xs, i, incx
         ieq = ii + (jj-1)*sdnx
         ks(ieq,jeq,n,oct) = zbcyz(ii,jj)
      END DO
   END DO
   DO kk = zs, k, incz
      DO ii = xs, i, incx
         ieq = xys + ii + (kk-1)*sdnx
         ks(ieq,jeq,n,oct) = ybcyz(ii,kk)
      END DO
   END DO
   DO kk = zs, k, incz
      DO jj = ys, j, incy
         ieq = (xys+xzs) + jj + (kk-1)*sdny
         ks(ieq,jeq,n,oct) = xbcyz(jj,kk)
      END DO
   END DO
END IF
