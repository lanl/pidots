SUBROUTINE idotigb(bit,phi,sv,psii,psio,av,jmat,kmat,jpsi,kpsi,piv)
...
! Only solve for phi if past the first iteration
IF (bit == 1) THEN
   t = sv
   DO j = 1, 8
      DO i = 1, neq
         t(i) = t(i) + DOT_PRODUCT(kmat(:,i,j),psii(:,j))
      END DO
   END DO

   CALL dgetrs('T',neq,1,jmat,neq,piv,t,neq,info)
   IF (info /= 0) THEN
