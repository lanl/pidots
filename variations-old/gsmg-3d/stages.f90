...
DO
   ...
   END IF
END DO
! Compute ns, including 1st coarsening step per processor
ns = 2 + (tt+1)/3
ns1 = ns - 1
ns2 = ns - 2
...
IF (iexit == 0) THEN
   ...
   ! Index by zero to keep all the parallel parameters unaffected from PBJ code
   ALLOCATE(xcf(0:ns2),ycf(0:ns2),zcf(0:ns2))

   ! Set the first coarsening factor coarsen all the sub-domains per processor
   xcf(0) = nxsd
   ycf(0) = nysd
   zcf(0) = nzsd
   ...
   t = npx
   DO i = 1, ns2
      ! Coarsen by 8 if evenly divisible by 8
      IF (MOD(t,8) == 0) THEN
         xcf(i) = 8
         t = t/8
      ! Coarsen by 4 if evenly divisible by 4
      ELSE IF (MOD(t,4) == 0) THEN
         xcf(i) = 4
         t = t/4
