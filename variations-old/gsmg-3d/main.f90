...
INTEGER :: statin, statout, ferr, ierr, iexit, i
INTEGER, DIMENSION(30) :: ipak
...
ELSE
   ...
   CALL MPI_BCAST(ipak,30,MPI_INTEGER,root,MPI_COMM_WORLD,ierr)
   nx     = ipak(1)
   ny     = ipak(2)
   nz     = ipak(3)
   npx    = ipak(4)
   ...
   sfp    = ipak(29)
   iexit  = ipak(30)
   ! Other processes receive the packaged coarsening arrays
   i = 3*ns - 3
   ALLOCATE(cpak(i))
   ALLOCATE(xcf(0:ns2), ycf(0:ns2), zcf(0:ns2))
   CALL MPI_BCAST(cpak,i,MPI_INTEGER,root,MPI_COMM_WORLD,ierr)
   xcf = cpak(1:(ns-1))
   ycf = cpak(ns:(2*ns-2))
   zcf = cpak((2*ns-1):i)
   DEALLOCATE(cpak)
...
! Allocate flux and the total flux solution if necessary
ALLOCATE(flux(nx,ny,nz))
IF (irank == root) THEN
   IF (sfp == 1) ALLOCATE(fluxt(nxt,nyt,nzt))
END IF
...
DEALLOCATE(dx,dy,dz,mat,ang,w,sigt,sigs,s,xcf,ycf,zcf)
DEALLOCATE(flux)
