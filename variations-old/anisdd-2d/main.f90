...
!   2D Anisotropic Scattering DD code
!
!-----------------------------------------------------------------------------------
...
ELSE
   ...
   CALL MPI_BCAST(ipak,18,MPI_INTEGER,root,MPI_COMM_WORLD,ierr)
   meth   = ipak(1)
   tpose  = ipak(2)
   sym    = ipak(3)
   idos   = ipak(4)
   ...
   CALL MPI_BCAST(bc,4,MPI_INTEGER,root,MPI_COMM_WORLD,ierr)
   ...
   ALLOCATE(ang(apo,2), omg(apo), w(apo), sigt(nm,ng), sigs(0:anord,nm,ng,ng))
   CALL MPI_BCAST(ang,(2*apo),MPI_DOUBLE_PRECISION,root,MPI_COMM_WORLD,ierr)
   CALL MPI_BCAST(omg,apo,MPI_DOUBLE_PRECISION,root,MPI_COMM_WORLD,ierr)
   ...
   CALL MPI_BCAST(sigs,(ng*ng*nm*(anord+1)),MPI_DOUBLE_PRECISION,root,MPI_COMM_WORLD,ierr)
   ! Unpack rpak
   err = rpak(1)
   cgr = rpak(2)
   tolr = rpak(3)
...
IF (irank == root) THEN
   ...
   DEALLOCATE(dxt,dyt,matt,st)
   DEALLOCATE(posiny,neginy,posinx,neginx)
END IF
...
IF (irank == root) DEALLOCATE(ssum,nxvec,nyvec)
