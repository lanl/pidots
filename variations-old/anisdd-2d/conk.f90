SUBROUTINE conk(n,i,j,incx,incy,xs,ys,mu,omega,ktmpy,ktmpx)
...
REAL*8, DIMENSION(nmom,nx), INTENT(OUT) :: ktmpy
REAL*8, DIMENSION(nmom,ny), INTENT(OUT) :: ktmpx
...
IF (j /= ys) THEN
   DO ii = xs, i-incx, incx
      ktmpy(1,ii) = ktmpy(1,ii) + wt*gay*ybcyo(ii)
   END DO
   DO ii = xs, i, incx
      ktmpy(1,ii) = ktmpy(1,ii) + wt*gax*ybcxo(ii)
   END DO
ELSE IF (j == ys) THEN
   DO ii = xs, i, incx
      IF (ii /= i) THEN
...
IF (i /= xs) THEN
   DO jj = ys, j, incy
      ktmpx(1,jj) = ktmpx(1,jj) + wt*gay*xbcyo(jj)
   END DO
   DO jj = ys, j-incy, incy
      ktmpx(1,jj) = ktmpx(1,jj) + wt*gax*xbcxo(jj)
   END DO
ELSE IF (i == xs) THEN
   DO jj = ys, j, incy
      IF (jj /= j) THEN
