SUBROUTINE conkt(n,i,j,k,incx,incy,incz,xs,ys,zs,mu,omega,ktmpz,ktmpy,ktmpx)
...
REAL*8, DIMENSION(nx*ny,nmom), INTENT(OUT) :: ktmpz
REAL*8, DIMENSION(nx*nz,nmom), INTENT(OUT) :: ktmpy
REAL*8, DIMENSION(ny*nz,nmom), INTENT(OUT) :: ktmpx
...
IF (k /= zs) THEN
   DO jj = ys, j, incy
      DO ii = xs, i-incx, incx
         ieq = ii + (jj-1)*nx
         ktmpz(ieq,1) = ktmpz(ieq,1) + wt*gayz*zbcyzo(ii,jj)
      END DO
   END DO
   DO jj = ys, j-incy, incy
      DO ii = xs, i, incx
         ieq = ii + (jj-1)*nx
         ktmpz(ieq,1) = ktmpz(ieq,1) + wt*gaxz*zbcxzo(ii,jj)
      END DO
   END DO
   DO jj = ys, j, incy
      DO ii = xs, i, incx
         ieq = ii + (jj-1)*nx
         ktmpz(ieq,1) = ktmpz(ieq,1) + wt*gaxy*zbcxyo(ii,jj)
      END DO
   END DO
ELSE IF (k == zs) THEN
   DO jj = ys, j, incy
      DO ii = xs, i, incx
         ...
         IF (ii == i .AND. jj == j) THEN
            ktmpz(ieq,1) = ktmpz(ieq,1) + wt*gaxy
         END IF
      END DO
   END DO
END IF
...
IF (j /= ys) THEN
   DO kk = zs, k, incz
      DO ii = xs, i-incx, incx
         ieq = ii + (kk-1)*nx
         ktmpy(ieq,1) = ktmpy(ieq,1) + wt*gayz*ybcyzo(ii,kk)
      END DO
   END DO
   DO kk = zs, k, incz
      DO ii = xs, i, incx
         ieq = ii + (kk-1)*nx
         ktmpy(ieq,1) = ktmpy(ieq,1) + wt*gaxz*ybcxzo(ii,kk)
      END DO
   END DO
   DO kk = zs, k-incz, incz
      DO ii = xs, i, incx
         ieq = ii + (kk-1)*nx
         ktmpy(ieq,1) = ktmpy(ieq,1) + wt*gaxy*ybcxyo(ii,kk)
      END DO
   END DO
ELSE IF (j == ys) THEN
   DO kk = zs, k, incz
      DO ii = xs, i, incx
         ...
         IF (ii == i .AND. kk == k) THEN
            ktmpy(ieq,1) = ktmpy(ieq,1) + wt*gaxz
         END IF
      END DO
   END DO
END IF
...
IF (i /= xs) THEN
   DO kk = zs, k, incz
      DO jj = ys, j, incy
         ieq = jj + (kk-1)*ny
         ktmpx(ieq,1) = ktmpx(ieq,1) + wt*gayz*xbcyzo(jj,kk)
      END DO
   END DO
   DO kk = zs, k, incz
      DO jj = ys, j-incy, incy
         ieq = jj + (kk-1)*ny
         ktmpx(ieq,1) = ktmpx(ieq,1) + wt*gaxz*xbcxzo(jj,kk)
      END DO
   END DO
   DO kk = zs, k-incz, incz
      DO jj = ys, j, incy
         ieq = jj + (kk-1)*ny
         ktmpx(ieq,1) = ktmpx(ieq,1) + wt*gaxy*xbcxyo(jj,kk)
      END DO
   END DO
ELSE IF (i == xs) THEN
   DO kk = zs, k, incz
      DO jj = ys, j, incy
         ...
         IF (jj == j .AND. kk == k) THEN
            ktmpx(ieq,1) = ktmpx(ieq,1) + wt*gayz
         END IF
      END DO
   END DO
END IF
