...
REAL*8 :: x, y, z, mu, eta, xi, omega, sig, pi, tmp
REAL*8, DIMENSION(0:anord) :: tsxs

tmp = 0.0
pi = 2.0*ACOS(tmp)

! Allocate the four vector pieces of the gamma matrix
ALLOCATE(gaa(nmom), gxya(nmom), gxza(nmom), gyza(nmom))

! Allocate X-xmat and Y-ymat and Z-zmat and their 'old' counterparts
ALLOCATE(xmat(nmom,nx,ny,nz), ymat(nmom,nx,ny,nz,nx), zmat(nmom,nx,ny,nz,nx,ny))
ALLOCATE(xold(nmom,nx,ny,nz), yold(nmom,nx,ny,nz), zold(nmom,nx,ny,nz))
      ...
      DO k = zs, ze, incz
         ...
         DO j = ys, ye, incy
            ...
            DO i = xs, xe, incx
               ...
               CALL jima(i,j,k,xs,ys,zs,xe,ye,ze,incx,incy,incz,n,oct,mu,omega,tsxs)

               ! Call afcm to set up matrices for psi_in vector
               CALL afcm(i,j,k,xs,ys,zs,xe,ye,ze,incx,incy,incz,n,oct,mu,omega)
