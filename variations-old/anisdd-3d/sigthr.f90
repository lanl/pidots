...
ELSE
   ...
   DO pk = 0, npz-1
      ...
      DO pj = 0, npy-1
         ...
         DO pi = 0, npx-1
            coord(1) = pi
            eqs = nxvec(pi)*nyvec(pj)*nzvec(pk)*ng*nmom
            CALL MPI_CART_RANK(allcomm,coord,trank,ierr)
            IF (trank /= nrank) THEN
               ALLOCATE(tmpf(nmom,nxvec(pi),nyvec(pj),nzvec(pk),ng))
               tmpf = 0.0
               tag = 100 + trank
               ...
               DO g = 1, ng
                  DO k = 1, nzvec(pk)
                     DO j = 1, nyvec(pj)
                        DO i = 1, nxvec(pi)
                           flux(i+indx,j+jndx,k+kndx,g) = tmpf(1,i,j,k,g)
                        END DO
                     END DO
                  END DO
               END DO
            ...
            ELSE IF (trank == nrank) THEN
               ...
               DO g = 1, ng
                  DO k = 1, nz
                     DO j = 1, ny
                        DO i = 1, nx
                           flux(i+indx,j+jndx,k+kndx,g) = f(1,i,j,k,g)
                        END DO
                     END DO
                  END DO
               END DO
