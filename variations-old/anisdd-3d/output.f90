   ...
   ELSE
      DO g = 1, ng
         ...
         DO k = 1, nz
            DO j = 1, ny
               WRITE (8,*) " Plane(z) : ", k, " Row(j) : ", j
               ieq = ((k-1)*nx*ny + (j-1)*nx)*nmom
               jeq = ((k-1)*nx*ny + j*nx)*nmom
               WRITE (8,113) (phi(i,g), i = ieq+1, jeq, nmom)
!               WRITE (8,113) (phi(((k-1)*nx*ny+(j-1)*nx)*nmom+i,g), i = 1, ((k-1)*nx*ny+j*nx)*nmom, nmom)
            END DO
         END DO
      END DO
   END IF
