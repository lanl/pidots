...
ELSE
   ...
   DO pk = 0, npz-1
      ...
      DO pj = 0, npy-1
         ...
         DO pi = 0, npx-1
            coord(1) = pi
            eqs = nxvec(pi)*nyvec(pj)*nzvec(pk)*nmom
            ...
            IF (trank /= nrank) THEN
               ...
               DO g = 1, ng
                  DO k = 1, nzvec(pk)
                     DO j = 1, nyvec(pj)
                        DO i = 1, nxvec(pi)
                           ieq = ((i-1) + (j-1)*nxvec(pi) + (k-1)*nxvec(pi)*nyvec(pj))*nmom + 1
                           flux(i+indx,j+jndx,k+kndx,g) = tmphi(ieq,g)
                        END DO
                     END DO
                  END DO
               END DO
            ...
            ELSE IF (trank == nrank) THEN
               ...
               DO g = 1, ng
                  DO k = 1, nz
                     DO j = 1, ny
                        DO i = 1, nx
                           ieq = ((i-1) + (j-1)*nx + (k-1)*nx*ny)*nmom + 1
                           flux(i+indx,j+jndx,k+kndx,g) = phi(ieq,g)
                        END DO
                     END DO
                  END DO
               END DO
