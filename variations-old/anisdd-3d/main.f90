...
!   Update old version for factorization without inversion for direct method
!   Use MPI_WTIME
!
!-----------------------------------------------------------------------------------
...
ELSE
   ...
   CALL MPI_BCAST(ipak,20,MPI_INTEGER,root,MPI_COMM_WORLD,ierr)
   meth   = ipak(1)
   tpose  = ipak(2)
   sym    = ipak(3)
   idos   = ipak(4)
   ...
   ALLOCATE(ang(apo,3), omg(apo), w(apo), sigt(nm,ng), sigs(0:anord,nm,ng,ng))
