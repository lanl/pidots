SUBROUTINE afcm(i,j,k,xs,ys,zs,xe,ye,ze,incx,incy,incz,n,oct,v)
...
USE solvar
IMPLICIT NONE
INTEGER, INTENT(IN) :: i, j, k, xs, ys, zs, xe, ye, ze, incx, incy, incz, n, oct, v
...
indx = (n-1)*xys
kmat(indx+1:indx+xys,jeq,oct,v) = kmat(indx+1:indx+xys,jeq,oct,v) + ktmpz
indx = apo*xys + (n-1)*xzs
kmat(indx+1:indx+xzs,jeq,oct,v) = kmat(indx+1:indx+xzs,jeq,oct,v) + ktmpy
indx = apo*(xys+xzs) + (n-1)*yzs
kmat(indx+1:indx+yzs,jeq,oct,v) = kmat(indx+1:indx+yzs,jeq,oct,v) + ktmpx
...
IF (k == ze) THEN
   ...
   DO jj = ys, j, incy
      DO ii = xs, i, incx
         ieq = ii + (jj-1)*nx
         kpsi(ieq,jeq,n,oct,v) = zbcxy(ii,jj,i,j)
      END DO
   END DO
   DO kk = zs, k, incz
      DO ii = xs, i, incx
         ieq = xys + ii + (kk-1)*nx
         kpsi(ieq,jeq,n,oct,v) = ybcxy(ii,kk,i,j)
      END DO
   END DO
   DO kk = zs, k, incz
      DO jj = ys, j, incy
         ieq = (xys+xzs) + jj + (kk-1)*ny
         kpsi(ieq,jeq,n,oct,v) = xbcxy(jj,kk,i,j)
      END DO
   END DO
END IF
...
IF (j == ye) THEN
   ...
   DO jj = ys, j, incy
      DO ii = xs, i, incx
         ieq = ii + (jj-1)*nx
         kpsi(ieq,jeq,n,oct,v) = zbcxz(ii,jj,i)
      END DO
   END DO
   DO kk = zs, k, incz
      DO ii = xs, i, incx
         ieq = xys + ii + (kk-1)*nx
         kpsi(ieq,jeq,n,oct,v) = ybcxz(ii,kk,i)
      END DO
   END DO
   DO kk = zs, k, incz
      DO jj = ys, j, incy
         ieq = (xys+xzs) + jj + (kk-1)*ny
         kpsi(ieq,jeq,n,oct,v) = xbcxz(jj,kk,i)
      END DO
   END DO
END IF
...
IF (i == xe) THEN
   ! Reset equation index
   jeq = (xys+xzs) + j + (k-1)*ny
   DO jj = ys, j, incy
      DO ii = xs, i, incx
         ieq = ii + (jj-1)*nx
         kpsi(ieq,jeq,n,oct,v) = zbcyz(ii,jj)
      END DO
   END DO
   DO kk = zs, k, incz
      DO ii = xs, i, incx
         ieq = xys + ii + (kk-1)*nx
         kpsi(ieq,jeq,n,oct,v) = ybcyz(ii,kk)
      END DO
   END DO
   DO kk = zs, k, incz
      DO jj = ys, j, incy
         ieq = (xys+xzs) + jj + (kk-1)*ny
         kpsi(ieq,jeq,n,oct,v) = xbcyz(jj,kk)
      END DO
   END DO
END IF
