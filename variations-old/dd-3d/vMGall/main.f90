...
!    Use an initial guess from previous iteration on the CG method.
!    Move CG operations so convergence check takes place immediately and move
!       operations to within appropriate IF statement.
!    New routine SOR.
!
!
!-----------------------------------------------------------------------------------
...
ELSE
   ...
   CALL MPI_BCAST(ipak,22,MPI_INTEGER,root,MPI_COMM_WORLD,ierr)
   iexit  = ipak(1)
   tpose  = ipak(2)
   sym    = ipak(3)
   idos   = ipak(4)
