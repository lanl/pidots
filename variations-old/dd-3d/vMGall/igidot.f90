...
IF (idos == 0) THEN
   ...
   IF (tpose == 1) THEN
      ...
      END DO
   ELSE
      phi = sv + MATMUL(kmat(:,:,1),psii(:,1)) + MATMUL(kmat(:,:,2),psii(:,2)) + MATMUL(kmat(:,:,3),psii(:,3)) &
   ...
   IF (invf == 0) THEN
      IF (tpose == 1) THEN
         IF (sym == 1) THEN
            CALL dpotrs('U',neq,1,jmat,neq,phi,neq,info)
            IF (info /= 0) THEN
      ...
      ELSE
         IF (sym == 1) THEN
            ...
            END IF
         ELSE
            CALL dgetrs('N',neq,1,jmat,neq,piv,phi,neq,info)
            IF (info /= 0) THEN
               WRITE (8,'(//,1X,A)') "ERROR: dgetrs cannot solve."
               STOP
            END IF
...
ELSE
   ...
   ELSE
      IF (idos == 1) THEN
         ...
         ELSE
            IF (pcty == 0) THEN
               CALL cgsolvejac(q,its)
            ELSE IF (pcty == 1) THEN
               CALL cgsolvesgs(q,its)
            ELSE
               CALL cgsolvesor(q,its)
            END IF
         END IF
      ELSE
         CALL sor(q,its)
...
IF (tpose == 1) THEN
   DO j = 1, 8
      DO i = 1, bcs
         psio(i,j) = DOT_PRODUCT(jpsi(:,i,j),q)
      END DO
      DO i = 1, apo
         kz = (i-1)*xys + 1
         lz = i*xys
...
ELSE
   DO j = 1, 8
      ...
      DO i = 1, apo
         ...
         lx = apo*(xys+xzs) + i*yzs
         ! zbc's in psio
         psio(kz:lz,j) = psio(kz:lz,j) + MATMUL(kpsi(ix1:ix2,ix1:ix2,i,j),psii(kz:lz,j)) &
                                       + MATMUL(kpsi(ix1:ix2,ix3:ix4,i,j),psii(ky:ly,j)) &
                                       + MATMUL(kpsi(ix1:ix2,ix5:ix6,i,j),psii(kx:lx,j))
         ! ybc's in psio
         psio(ky:ly,j) = psio(ky:ly,j) + MATMUL(kpsi(ix3:ix4,ix1:ix2,i,j),psii(kz:lz,j)) &
                                       + MATMUL(kpsi(ix3:ix4,ix3:ix4,i,j),psii(ky:ly,j)) &
                                       + MATMUL(kpsi(ix3:ix4,ix5:ix6,i,j),psii(kx:lx,j))
         ! xbc's in psio
         psio(kx:lx,j) = psio(kx:lx,j) + MATMUL(kpsi(ix5:ix6,ix1:ix2,i,j),psii(kz:lz,j)) &
