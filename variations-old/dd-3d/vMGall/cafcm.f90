SUBROUTINE cafcm(i,j,k,xs,ys,zs,xe,ye,ze,incx,incy,incz,n,oct)

!-------------------------------------------------------------
!
!  Angular Flux Coefficient Matrix
!  Directs the construction of the matrices for working with
!   the incoming angular flux. Constructs ckmat's and ckpsi's
!
!-------------------------------------------------------------
...
REAL*8, DIMENSION(cxys) :: ktmpz
REAL*8, DIMENSION(cxzs) :: ktmpy
REAL*8, DIMENSION(cyzs) :: ktmpx
...
! Update the appropriate ckmat given the octant of interest
! Call conk to construct the row of ckmat, then set it in ckmat#
! Set according to 'tpose' flag
IF (tpose == 1) THEN
   jeq = i + (j-1)*npx + (k-1)*cxys
   CALL cconk(n,i,j,k,incx,incy,incz,xs,ys,zs,ktmpz,ktmpy,ktmpx)
   indx = (n-1)*cxys
   ckmat(indx+1:indx+cxys,jeq,oct) = ckmat(indx+1:indx+cxys,jeq,oct) + ktmpz
   indx = apo*cxys + (n-1)*cxzs
   ckmat(indx+1:indx+cxzs,jeq,oct) = ckmat(indx+1:indx+cxzs,jeq,oct) + ktmpy
   indx = apo*(cxys+cxzs) + (n-1)*cyzs
   ckmat(indx+1:indx+cyzs,jeq,oct) = ckmat(indx+1:indx+cyzs,jeq,oct) + ktmpx

   ! Use the data in the xbc, ybc, zbc varialbes at xe ye ze to compute ckpsi
   ...
   IF (k == ze) THEN
      ...
      DO jj = ys, j, incy
         DO ii = xs, i, incx
            ieq = ii + (jj-1)*npx
            ckpsi(ieq,jeq,n,oct) = zbcxy(ii,jj,i,j)
         END DO
      END DO
      DO kk = zs, k, incz
         DO ii = xs, i, incx
            ieq = cxys + ii + (kk-1)*npx
            ckpsi(ieq,jeq,n,oct) = ybcxy(ii,kk,i,j)
         END DO
      END DO
      DO kk = zs, k, incz
         DO jj = ys, j, incy
            ieq = (cxys+cxzs) + jj + (kk-1)*npy
            ckpsi(ieq,jeq,n,oct) = xbcxy(jj,kk,i,j)
         END DO
      END DO
   END IF
   ...
   IF (j == ye) THEN
      ...
      DO jj = ys, j, incy
         DO ii = xs, i, incx
            ieq = ii + (jj-1)*npx
            ckpsi(ieq,jeq,n,oct) = zbcxz(ii,jj,i)
         END DO
      END DO
      DO kk = zs, k, incz
         DO ii = xs, i, incx
            ieq = cxys + ii + (kk-1)*npx
            ckpsi(ieq,jeq,n,oct) = ybcxz(ii,kk,i)
         END DO
      END DO
      DO kk = zs, k, incz
         DO jj = ys, j, incy
            ieq = (cxys+cxzs) + jj + (kk-1)*npy
            ckpsi(ieq,jeq,n,oct) = xbcxz(jj,kk,i)
         END DO
      END DO
   END IF
   ...
   IF (i == xe) THEN
      ! Reset equation index
      jeq = (cxys+cxzs) + j + (k-1)*npy
      DO jj = ys, j, incy
         DO ii = xs, i, incx
            ieq = ii + (jj-1)*npx
            ckpsi(ieq,jeq,n,oct) = zbcyz(ii,jj)
         END DO
      END DO
      DO kk = zs, k, incz
         DO ii = xs, i, incx
            ieq = cxys + ii + (kk-1)*npx
            ckpsi(ieq,jeq,n,oct) = ybcyz(ii,kk)
         END DO
      END DO
      DO kk = zs, k, incz
         DO jj = ys, j, incy
            ieq = (cxys+cxzs) + jj + (kk-1)*npy
            ckpsi(ieq,jeq,n,oct) = xbcyz(jj,kk)
         END DO
      END DO
   END IF
!-------------------------------------------------------------------------------
ELSE
   ...
   CALL cconk(n,i,j,k,incx,incy,incz,xs,ys,zs,ktmpz,ktmpy,ktmpx)
   indx = (n-1)*cxys
   ckmat(ieq,indx+1:indx+cxys,oct) = ckmat(ieq,indx+1:indx+cxys,oct) + ktmpz
   indx = apo*cxys + (n-1)*cxzs
   ckmat(ieq,indx+1:indx+cxzs,oct) = ckmat(ieq,indx+1:indx+cxzs,oct) + ktmpy
   indx = apo*(cxys+cxzs) + (n-1)*cyzs
   ckmat(ieq,indx+1:indx+cyzs,oct) = ckmat(ieq,indx+1:indx+cyzs,oct) + ktmpx
   ...
   IF (k == ze) THEN
      ...
      DO kk = zs, k, incz
         DO ii = xs, i, incx
            jeq = cxys + ii + (kk-1)*npx
            ckpsi(ieq,jeq,n,oct) = ybcxy(ii,kk,i,j)
         END DO
      END DO
      DO kk = zs, k, incz
         DO jj = ys, j, incy
            jeq = (cxys+cxzs) + jj + (kk-1)*npy
            ckpsi(ieq,jeq,n,oct) = xbcxy(jj,kk,i,j)
         END DO
      END DO
   END IF
