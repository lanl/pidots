SUBROUTINE cmatsweep

!-----------------------------------------------------------------
!
!  Coarse grid matrix sweep
!
...
ALLOCATE(xmat(npx,npy,npz), ymat(npx,npy,npz,npx), zmat(npx,npy,npz,npx,npy))
ALLOCATE(xold(npx,npy,npz), yold(npx,npy,npz), zold(npx,npy,npz))

! Allocate Boundary Condition Variables
ALLOCATE(zbcxy(npx,npy,npx,npy),ybcxy(npx,npz,npx,npy),xbcxy(npy,npz,npx,npy))
ALLOCATE(zbcxz(npx,npy,npx),ybcxz(npx,npz,npx),xbcxz(npy,npz,npx))
ALLOCATE(zbcyz(npx,npy),zbcxyo(npx,npy),zbcxzo(npx,npy),zbcyzo(npx,npy))
ALLOCATE(ybcyz(npx,npz),ybcxyo(npx,npz),ybcxzo(npx,npz),ybcyzo(npx,npz))
ALLOCATE(xbcyz(npy,npz),xbcxyo(npy,npz),xbcxzo(npy,npz),xbcyzo(npy,npz))
      ...
      DO k = zs, ze, incz
         ...
         DO j = ys, ye, incy
            ...
            DO i = xs, xe, incx
               ...
               CALL cjima(i,j,k,xs,ys,zs,xe,ye,ze,incx,incy,incz,n,c,oct)

               ! Call afcm to set up matrices for psi_in vector
               CALL cafcm(i,j,k,xs,ys,zs,xe,ye,ze,incx,incy,incz,n,oct)
