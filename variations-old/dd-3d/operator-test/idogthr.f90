...
IF (nrank /= root2) THEN
   tag = 100 + nrank
   CALL MPI_SEND(phi,neq,MPI_DOUBLE_PRECISION,root2,tag,allcomm,ierr)
ELSE
   kndx = 0
   DO pk = 0, npz-1
      coord(1) = pk
      ...
      DO pj = 0, npy-1
         ...
         DO pi = 0, npx-1
            ...
            IF (trank /= nrank) THEN
               ...
               tmphi = 0.0
               tag = 100 + trank
               CALL MPI_RECV(tmphi,eqs,MPI_DOUBLE_PRECISION,trank,tag,allcomm,istat,ierr)
               DO k = 1, nzvec(pk)
                  DO j = 1, nyvec(pj)
                     DO i = 1, nxvec(pi)
               ...
               DO k = 1, nz
                  DO j = 1, ny
                     DO i = 1, nx
                        ...
                        flux(i+indx,j+jndx,k+kndx) = phi(ieq)
                     END DO
                  END DO
               END DO
            END IF
