...
!    Trimmed version with no solution options (direct, factored, asymmetric,
!    with transpose only) and no groups.
!
!-----------------------------------------------------------------------------------
...
ELSE
   ...
   CALL MPI_BCAST(ipak,12,MPI_INTEGER,root,MPI_COMM_WORLD,ierr)
   nxt    = ipak(1)
   nyt    = ipak(2)
   nzt    = ipak(3)
   npx    = ipak(4)
...
DEALLOCATE(psiiz,psiiy,psiix,psioz,psioy,psiox)
DEALLOCATE(phi)
