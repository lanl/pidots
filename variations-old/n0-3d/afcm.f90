...
IF (tpose == 1) THEN
   ...
   indx = (n-1)*xys
   kmat(indx+1:indx+xys,jeq,oct) = kmat(indx+1:indx+xys,jeq,oct) + ktmpz
   indx = apo*xys + (n-1)*xzs
   kmat(indx+1:indx+xzs,jeq,oct) = kmat(indx+1:indx+xzs,jeq,oct) + ktmpy
   indx = apo*(xys+xzs) + (n-1)*yzs
   kmat(indx+1:indx+yzs,jeq,oct) = kmat(indx+1:indx+yzs,jeq,oct) + ktmpx
...
ELSE
   ...
   CALL conk(n,i,j,k,incx,incy,incz,xs,ys,zs,ktmpz,ktmpy,ktmpx)
   indx = (n-1)*xys
   kmat(ieq,indx+1:indx+xys,oct) = kmat(ieq,indx+1:indx+xys,oct) + ktmpz
   indx = apo*xys + (n-1)*xzs
   kmat(ieq,indx+1:indx+xzs,oct) = kmat(ieq,indx+1:indx+xzs,oct) + ktmpy
   indx = apo*(xys+xzs) + (n-1)*yzs
   kmat(ieq,indx+1:indx+yzs,oct) = kmat(ieq,indx+1:indx+yzs,oct) + ktmpx
