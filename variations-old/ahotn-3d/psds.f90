...
USE totvar
IMPLICIT NONE
INTEGER :: bnc, i, j, n, t, u, pi, pj, pk, tnx, tny, tnz, recrnk, sndrnk
      ...
      DO pj = 0, npy-1
         ...
         DO pi = 0, npx-1
            ...
            ELSE
               ...
               ALLOCATE(dx(nx), dy(ny), dz(nz), mat(nx,ny,nz), s(nx,ny,nz,0:lambda,0:lambda,0:lambda,ng))
               ALLOCATE(psii(bcs,8))
               dx = 0.0
               dy = 0.0
               dz = 0.0
   ...
   DO pk = 0, npz-1
      ...
      DO pj = 0, npy-1
         ...
         DO pi = 0, npx-1
            ...
            bcs = apo*(tnx*tny+tnx*tnz+tny*tnz)*ordsq
            ALLOCATE(tempmat(tnx,tny,tnz), temps(tnx,tny,tnz,0:lambda,0:lambda,0:lambda,ng))
            ...
            IF (pk == 0 .AND. bc(1) == 2) THEN
               DO n = 1, apo
                  indx1 = (n-1)*(tnx*tny)*ordsq
                  DO j = 1, tny
                     DO i = 1, tnx
                        DO t = 0, lambda
                           DO u = 0, lambda
                              ieq = indx1 + ((i-1) + (j-1)*tnx)*ordsq + t*order+ u + 1
                              tempsi(ieq,1) = posinz(indx+i,jndx+j,t,u,n,1)
                              tempsi(ieq,2) = posinz(indx+i,jndx+j,t,u,n,2)
                              tempsi(ieq,3) = posinz(indx+i,jndx+j,t,u,n,3)
                              tempsi(ieq,4) = posinz(indx+i,jndx+j,t,u,n,4)
                           END DO
                        END DO
                     END DO
                  END DO
                  ...
                  DO j = 1, tny
                     DO i = 1, tnx
                        DO t = 0, lambda
                           DO u = 0, lambda
                              ieq = indx1 + ((i-1) + (j-1)*tnx)*ordsq + t*order+ u + 1
                              tempsi(ieq,5) = neginz(indx+i,jndx+j,t,u,n,1)
                              tempsi(ieq,6) = neginz(indx+i,jndx+j,t,u,n,2)
                              tempsi(ieq,7) = neginz(indx+i,jndx+j,t,u,n,3)
                              tempsi(ieq,8) = neginz(indx+i,jndx+j,t,u,n,4)
                           END DO
                        END DO
                     END DO
                  END DO
            ...
            IF (pj == 0 .AND. bc(3) == 2) THEN
               DO n = 1, apo
                  indx1 = apo*(tnx*tny)*ordsq + (n-1)*(tnx*tnz)*ordsq
                  DO j = 1, tnz
                     DO i = 1, tnx
                        DO t = 0, lambda
                           DO u = 0, lambda
                              ieq = indx1 + ((i-1) + (j-1)*tnx)*ordsq + t*order+ u + 1
                              tempsi(ieq,1) = posiny(indx+i,kndx+j,t,u,n,1)
                              tempsi(ieq,2) = posiny(indx+i,kndx+j,t,u,n,2)
                              tempsi(ieq,5) = posiny(indx+i,kndx+j,t,u,n,3)
                              tempsi(ieq,6) = posiny(indx+i,kndx+j,t,u,n,4)
                           END DO
                        END DO
                     END DO
                  END DO
                  ...
                  DO j = 1, tnz
                     DO i = 1, tnx
                        DO t = 0, lambda
                           DO u = 0, lambda
                              ieq = indx1 + ((i-1) + (j-1)*tnx)*ordsq + t*order+ u + 1
                              tempsi(ieq,3) = neginy(indx+i,kndx+j,t,u,n,1)
                              tempsi(ieq,4) = neginy(indx+i,kndx+j,t,u,n,2)
                              tempsi(ieq,7) = neginy(indx+i,kndx+j,t,u,n,3)
                              tempsi(ieq,8) = neginy(indx+i,kndx+j,t,u,n,4)
                           END DO
                        END DO
                     END DO
                  END DO
            ...
            IF (pi == 0 .AND. bc(5) == 2) THEN
               DO n = 1, apo
                  indx1 = apo*(tnx*tny+tnx*tnz)*ordsq + (n-1)*(tny*tnz)*ordsq
                  DO j = 1, tnz
                     DO i = 1, tny
                        DO t = 0, lambda
                           DO u = 0, lambda
                              ieq = indx1 + ((i-1) + (j-1)*tny)*ordsq + t*order+ u + 1
                              tempsi(ieq,1) = posinx(jndx+i,kndx+j,t,u,n,1)
                              tempsi(ieq,4) = posinx(jndx+i,kndx+j,t,u,n,2)
                              tempsi(ieq,5) = posinx(jndx+i,kndx+j,t,u,n,3)
                              tempsi(ieq,8) = posinx(jndx+i,kndx+j,t,u,n,4)
                           END DO
                        END DO
                     END DO
                  END DO
                  ...
                  DO j = 1, tnz
                     DO i = 1, tny
                        DO t = 0, lambda
                           DO u = 0, lambda
                              ieq = indx1 + ((i-1) + (j-1)*tny)*ordsq + t*order+ u + 1
                              tempsi(ieq,2) = neginx(jndx+i,kndx+j,t,u,n,1)
                              tempsi(ieq,3) = neginx(jndx+i,kndx+j,t,u,n,2)
                              tempsi(ieq,6) = neginx(jndx+i,kndx+j,t,u,n,3)
                              tempsi(ieq,7) = neginx(jndx+i,kndx+j,t,u,n,4)
                           END DO
                        END DO
                     END DO
                  END DO
            ...
            IF (recrnk /= sndrnk) THEN
               ...
               CALL MPI_SEND(temps,tnx*tny*tnz*ordcb*ng,MPI_DOUBLE_PRECISION,recrnk,tags,allcomm,ierr)
            ...
            ELSE
               ...
               psii(1:(apo*(nx*ny+nx*nz+ny*nz)*ordsq),1:8) = tempsi(1:bcs,1:8)
            END IF
...
ELSE
   ...
   CALL MPI_RECV(s,nx*ny*nz*ordcb*ng,MPI_DOUBLE_PRECISION,sndrnk,tags,allcomm,istat,ierr)
