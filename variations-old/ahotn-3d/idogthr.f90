...
ELSE
   ...
   DO pk = 0, npz-1
      ...
      DO pj = 0, npy-1
         ...
         DO pi = 0, npx-1
            coord(1) = pi
            eqs = nxvec(pi)*nyvec(pj)*nzvec(pk)*ordcb
                     ...
                     DO u = 0, lambda
                        DO v = 0, lambda
                           DO k = 1, nzvec(pk)
                              DO j = 1, nyvec(pj)
                                 DO i = 1, nxvec(pi)
                                    ieq = ((i-1)+(j-1)*nxvec(pi)+(k-1)*nxvec(pi)*nyvec(pj))*ordcb+t*ordsq+u*order+v+1
                                    flux(i+indx,j+jndx,k+kndx,t,u,v,g) = tmphi(ieq,g)
                                 END DO
                              END DO
                           END DO
                        END DO
                           ...
                           DO k = 1, nz
                              DO j = 1, ny
                                 DO i = 1, nx
                                    ...
                                    flux(i+indx,j+jndx,k+kndx,t,u,v,g) = phi(ieq,g)
                                 END DO
                              END DO
                           END DO
                        END DO
