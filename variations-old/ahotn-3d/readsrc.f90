...
INTEGER :: form, a, ni, i, j, k, t, u, v, g, xs, xe, ys, ye, zs, ze
REAL*8 :: q
CHARACTER(8), INTENT(IN) :: srcfile

! Set up the size of the arrays for the cross sections
ALLOCATE(st(nxt,nyt,nzt,0:lambda,0:lambda,0:lambda,ng))
...
IF (form == 0) THEN
   ...
   !  g  => the group
   !  t  => x-moment (spatial) of the instruction
   !  u  => y-moment of the instruction
   !  v  => z-moment of the instruction
      ...
      DO v = 0, lambda
         DO u = 0, lambda
            DO t = 0, lambda
               ...
               DO k = 1, nzt
                  DO j = 1, nyt
                     READ(12,*) (st(i,j,k,t,u,v,g), i = 1, nxt)
                  END DO
               END DO
            END DO
         END DO
