...
REAL*8 :: df, dfmx, af, afmx
REAL*8, DIMENSION(nsdp) :: dfsd, afsd
REAL*8, INTENT(OUT) :: tsolve
...
!-------------------------------------------------------------------------------
! Old code for getting the worst cell
!
...
!-----------------------------------------------------------------------------------------
! Get max error of scalar and angular flux

! Check convergence of scalar flux
! Each process checks its own fluxes, then perform MPI Reduction
IF (igf == 1) THEN ! Source is zero, solution is zero, so error is current value of flux
   DO i = 1, nsdp
      dfsd(i) = MAXVAL(ABS(phi(:,i)))
   END DO
ELSE IF (MINVAL(ABS(phiold)) >= tolr) THEN
   DO i = 1, nsdp
      dfsd(i) = MAXVAL(ABS((phi(:,i) - phiold(:,i))/phiold(:,i)))
   END DO
ELSE
...
!---------------------------------------------------------------
! Check convergence of angular flux at boundaries
! Each process checks its own fluxes, then perform MPI Reduction
IF (igf == 1) THEN ! Source is zero, solution is zero, so error is current value of flux
   DO i = 1, nsdp
      afsd(i) = MAXVAL(ABS(psio(:,:,i)))
   END DO
ELSE IF (MINVAL(ABS(psiold)) >= tolr) THEN
   DO i = 1, nsdp
      afsd(i) = MAXVAL(ABS((psio(:,:,i) - psiold(:,:,i))/psiold(:,:,i)))
   END DO
ELSE
   DO i = 1, nsdp
      afsd(i) = MAXVAL(ABS(psio(:,:,i) - psiold(:,:,i)))
   END DO
END IF
af = MAXVAL(afsd)

! Get the overall max difference
CALL MPI_ALLREDUCE(af,afmx,1,MPI_DOUBLE_PRECISION,MPI_MAX,allcomm,ierr)

! Finished using the initial guess, so reset psii to zero before doing the copies/comms
IF (bit == 1 .and. igf == 1) psii = 0.d0

IF (dfmx > err .AND. afmx > err .AND. bit < bitmx) THEN
   ! Only print iteration information if requested
   IF (irank == root) THEN
      IF (itp == 1) WRITE(8,111) bit, dfmx, afmx
   END IF
...
ELSE IF (dfmx < err .OR. afmx < err) THEN
   bcnvf = 1
   tsolve = MPI_WTIME()
   IF (irank == root) THEN
      WRITE(8,'(2X,A,I5,A,ES11.3,A,ES11.3,/)') "System converged in ", bit,          &
                  " iterations. Max phi error = ", dfmx, " Max psi error = ", afmx
  END IF
ELSE IF (bit == bitmx) THEN
   bcnvf = 0
   tsolve = MPI_WTIME()
   IF (irank == root) THEN
      WRITE (8,'(/,2X,A,I5,A,ES11.3,A,ES11.3)') "ERROR: System did not converge in ", bit,    &
                  " iterations . Max phi error = ", dfmx, " Max psi error = ", afmx
      WRITE (8,*) "Will adversely affect other results. Aborting program."
   END IF
END IF

111 FORMAT(2X,'It ', I5,' Dfmx ', ES11.3,' Afmx ', ES11.3)
