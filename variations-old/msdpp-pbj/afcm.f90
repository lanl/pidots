SUBROUTINE afcm(sdi,i,j,k,xs,ys,zs,xe,ye,ze,incx,incy,incz,n,oct)
...
USE solvar
IMPLICIT NONE
INTEGER, INTENT(IN) :: sdi, i, j, k, xs, ys, zs, xe, ye, ze, incx, incy, incz, n, oct
...
indx = (n-1)*xys
kmat(indx+1:indx+xys,jeq,oct,sdi) = kmat(indx+1:indx+xys,jeq,oct,sdi) + ktmpz
indx = apo*xys + (n-1)*xzs
kmat(indx+1:indx+xzs,jeq,oct,sdi) = kmat(indx+1:indx+xzs,jeq,oct,sdi) + ktmpy
indx = apo*(xys+xzs) + (n-1)*yzs
kmat(indx+1:indx+yzs,jeq,oct,sdi) = kmat(indx+1:indx+yzs,jeq,oct,sdi) + ktmpx
...
IF (k == ze) THEN
   ...
   DO jj = ys, j, incy
      DO ii = xs, i, incx
         ieq = ii + (jj-1)*sdnx
         kpsi(ieq,jeq,n,oct,sdi) = zbcxy(ii,jj,i,j)
      END DO
   END DO
   DO kk = zs, k, incz
      DO ii = xs, i, incx
         ieq = xys + ii + (kk-1)*sdnx
         kpsi(ieq,jeq,n,oct,sdi) = ybcxy(ii,kk,i,j)
      END DO
   END DO
   DO kk = zs, k, incz
      DO jj = ys, j, incy
         ieq = (xys+xzs) + jj + (kk-1)*sdny
         kpsi(ieq,jeq,n,oct,sdi) = xbcxy(jj,kk,i,j)
      END DO
   END DO
END IF
...
IF (j == ye) THEN
   ...
   DO jj = ys, j, incy
      DO ii = xs, i, incx
         ieq = ii + (jj-1)*sdnx
         kpsi(ieq,jeq,n,oct,sdi) = zbcxz(ii,jj,i)
      END DO
   END DO
   DO kk = zs, k, incz
      DO ii = xs, i, incx
         ieq = xys + ii + (kk-1)*sdnx
         kpsi(ieq,jeq,n,oct,sdi) = ybcxz(ii,kk,i)
      END DO
   END DO
   DO kk = zs, k, incz
      DO jj = ys, j, incy
         ieq = (xys+xzs) + jj + (kk-1)*sdny
         kpsi(ieq,jeq,n,oct,sdi) = xbcxz(jj,kk,i)
      END DO
   END DO
END IF
...
IF (i == xe) THEN
   ! Reset equation index
   jeq = (xys+xzs) + j + (k-1)*sdny
   DO jj = ys, j, incy
      DO ii = xs, i, incx
         ieq = ii + (jj-1)*sdnx
         kpsi(ieq,jeq,n,oct,sdi) = zbcyz(ii,jj)
      END DO
   END DO
   DO kk = zs, k, incz
      DO ii = xs, i, incx
         ieq = xys + ii + (kk-1)*sdnx
         kpsi(ieq,jeq,n,oct,sdi) = ybcyz(ii,kk)
      END DO
   END DO
   DO kk = zs, k, incz
      DO jj = ys, j, incy
         ieq = (xys+xzs) + jj + (kk-1)*sdny
         kpsi(ieq,jeq,n,oct,sdi) = xbcyz(jj,kk)
      END DO
   END DO
END IF
