...
ALLOCATE(psio(bcs,8,nsdp),psiold(bcs,8,nsdp))
...
! Do PBJ iterations, NOT PGS. So skip red iterations.
! Do everything under the black iterations and fix pgsblk.f90 to handle all the
! communications
DO bit = 1, bitmx
...
DEALLOCATE(phi,phiold,src,sv,piv,jmat,kmat,jpsi,kpsi,psio,psiold)
