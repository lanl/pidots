...
ELSE IF (MINVAL(ang) <= 0.d0 .OR. MAXVAL(ang) >= 1.d0) THEN
   ...
   iexit = iexit + 1
ELSE IF (MINVAL(w) <= 0.d0 .OR. MAXVAL(w) > 0.125d0) THEN
   WRITE(8,'(/,3X,A)') "ERROR: Illegal value for weight. Must be entered positive, less than 0.125."
   iexit = iexit + 1
ELSE IF (MINVAL(leng) <= 0.99d0 .OR. MAXVAL(leng) >= 1.01d0) THEN
...
! Checks on the initial guess data
ELSE IF (igf < 0 .OR. igf > 1) THEN
   WRITE(8,'(/,3X,A)') "ERROR: Illegal value for initial guess flag, igf. Must be 0/1=No/Yes initial guess for psi."
   iexit = iexit + 1
ELSE IF (igpsi < 0.d0) THEN
   WRITE(8,'(/,3X,A)') "ERROR: Illegal value for psi initial guess. Must be non-negative."
   iexit = iexit + 1
